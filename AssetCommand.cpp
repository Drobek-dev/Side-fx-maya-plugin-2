...
AssetCommand::doIt(const MArgList &args)
{
    ...
    MStatus ret = mySubCommand->doIt();
    /*auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = end - start;
    MGlobal::displayInfo("Asset command elapsed: " + MString(std::to_string(elapsed.count()).c_str()));
    MGlobal::displayInfo("Before reset: " + MString(std::to_string(Util::meshComputeElapsed.count()).c_str()));
    MGlobal::displayInfo("Before reset: " + MString(std::to_string(Util::meshConnectionElapsed.count()).c_str()));
    MGlobal::displayInfo("mesh elapsed: " + MString(std::to_string((Util::meshComputeElapsed + Util::meshConnectionElapsed).count()).c_str()));
    Util::meshComputeElapsed = std::chrono::duration<double>::zero();
    Util::meshConnectionElapsed = std::chrono::duration<double>::zero();
    MGlobal::displayInfo("After reset: " + MString(std::to_string(Util::meshComputeElapsed.count()).c_str()));
    MGlobal::displayInfo("After reset: " + MString(std::to_string(Util::meshConnectionElapsed.count()).c_str()));*/
    return ret;
}
